    ...
    if (data.Size == 1) {
      ...
      for (int offset = 0; offset < data.Size - 1; offset++) {
        ...
        if (Vector3.Dot(T, sD_TN) >= 0F) {
          pitchCorrection = Quaternion.AngleAxis(Mathf.Asin(T_x_sD_TN.magnitude) * RAD_2_DEG, T_x_sD_TN.normalized);
        }
        else {
          pitchCorrection = Quaternion.AngleAxis(180F - (Mathf.Asin(T_x_sD_TN.magnitude) * RAD_2_DEG), T_x_sD_TN.normalized);
        }
        ...
        Quaternion rollCorrection = Quaternion.AngleAxis(
          DeadzoneDampenFilter(canvasCastNB.magnitude) * Mathf.Asin(N_x_canvasNB.magnitude) * RAD_2_DEG,
