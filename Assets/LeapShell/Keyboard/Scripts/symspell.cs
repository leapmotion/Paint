...
static class SymSpell
{
    ...
    private static IEnumerable<string> parseWords(string text)
    {
      ...
      return text.Split(' ');//Regex.Matches(text.ToLower(), @"[\w-[\d_]]+").Cast<Match>().Select(m => m.Value);
    }
    ...
    private static bool CreateDictionaryEntry(string key, ulong index, string language)
    {
        bool result = false;
        ...
        else if (wordlist.Count < Int32.MaxValue)
        {
            ...
            (value as dictionaryItem).index = index;
    ...
    public static void CreateDictionary(string corpus, string language)
    {
        ...
        {
            UnityEngine.Debug.Log("File not found: " + corpus);
            return;
        }

        UnityEngine.Debug.Log("Creating dictionary ...");
        ...
        using (StreamReader sr = new StreamReader(corpus))
        {
            ...
            while ((line = sr.ReadLine()) != null)
            {
                ...
                {
                   iterationCount++;
                   if (CreateDictionaryEntry(key, iterationCount, language)) wordCount++;
                }
            }
        }
        wordlist.TrimExcess();
        stopWatch.Stop();
        UnityEngine.Debug.Log("\rDictionary: " + wordCount.ToString("N0") + " words, " + dictionary.Count.ToString("N0") + " entries, edit distance=" + editDistanceMax.ToString() + " in " + stopWatch.ElapsedMilliseconds.ToString()+"ms "+ (Process.GetCurrentProcess().PrivateMemorySize64/1000000).ToString("N0")+ " MB");
    }
    ...
    public static string Correct(string input, string language)
    {
        ...
        */
        ...
        //{
          //UnityEngine.Debug.Log(suggestion.term + " " + suggestion.distance.ToString() + " " + suggestion.count.ToString() + " " + stopWatch.Elapsed.TotalMilliseconds);
        //}
        //if (verbose !=0) UnityEngine.Debug.Log(suggestions.Count.ToString() + " suggestions");
